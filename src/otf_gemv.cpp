...
extern "C" void otf_gemv_kernel(int64_t n, int mb, int nb, double alpha,
    ...
    // Note that the sub-matrix cannot have diagonals. Use serial code for the
    // part includes diagonals instead.
...
// 24x24
//    alpha *= 0x1.fffffffffffffP-73;
// 96x96
//    alpha *= 0x1.fffffffffffffP-75;
    ...
    for (int64_t i = 0; i < mb; i += nn) {
        ...
        svfloat64_t y21 = sv0;
        svfloat64_t y31 = sv0;
        svint64_t sc00, sc10, sc20, sc30, sc01, sc11, sc21, sc31;
        {
            svint64_t a, c;
            a = svdup_s64(c8.a);
            c = svdup_s64(c8.c);
...
extern "C" void hmg_gemv_up(int istart, double a, double b, int mb, int nb,
                            double alpha, double const *__restrict__ x,
                            double *__restrict__ y) {
    // same as abobe, but for the upper-triangular part of the Higham's hpl-ai
    // matrix. istart is the row position of the sub-matrix. jstart is not
    // needed because of the structure of the matrix. a and b are the paramter
    // of the Higham's matrix.
    const int vlen = svcntd();
    const int nn = svcntd() * 4;
    auto t64 = svptrue_b64();
    double ab = a * b;
    ...
    for (int64_t i = 0; i < mb; i += nn) {
        ...
        svfloat64_t y21 = sv0;
        svfloat64_t y31 = sv0;
        // a+ab*(j+nn) = (a+ab*j) + ab*nn  is better for performance, but we
        // recompute them for accuracy
...
extern "C" void hmg_gemv_low(int istart, double a, double b, int mb, int nb,
                             double alpha, double const *__restrict__ x,
                             double *__restrict__ y) {
...
extern "C" void hmg_gemv_diag(int istart, int jstart, double a, double b,
                              int mb, int nb, double alpha,
                              double const *__restrict__ x,
                              double *__restrict__ y) {
