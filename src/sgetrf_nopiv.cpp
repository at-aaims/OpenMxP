...
void sgetrf_nopiv_naive(int m, int n, float *aptr, size_t lda) {
    auto a = [aptr, lda](int i, int j) -> float & { return aptr[i + lda * j]; };
...
template <int n> void sgetrf_nopiv_inner(int m, float *a, size_t lda) {
    sgetrf_nopiv_naive(m, n, a, lda);
}

template <int m>
void strsmLL_inner(const int n, const float *ap, float *bp, size_t ld) {
    auto a = [ap, ld](int i, int j) -> const float & { return ap[i + ld * j]; };
    auto b = [bp, ld](int i, int j) -> float & { return bp[i + ld * j]; };
    for (int j = 0; j < n; j++) {
...
template <int K>
void sgemm_inner(const int m, const int n, const float *ap, const float *bp,
                 float *cp, size_t ld) {
    if (0 == n)
        return;
...
void strsmLL_inner<6>(const int n, const float *ap, float *bp, size_t ld) {
    ...
    float a50 = a(5, 0), a51 = a(5, 1), a52 = a(5, 2), a53 = a(5, 3),
          a54 = a(5, 4);
...
template <>
void sgemm_inner<3>(const int m, const int n, const float *ap, const float *bp,
                    float *cp, size_t ld) {
    if (0 == n)
        return;
...
template <>
void sgemm_inner<6>(const int m, const int n, const float *__restrict__ ap,
...
template <> void sgetrf_nopiv_inner<3>(int m, float *aptr, size_t lda) {
    if (m < 2)
        return;
...
template <> void sgetrf_nopiv_inner<6>(int m, float *aptr, size_t lda) {
    auto a = [aptr, lda](int i, int j) -> float & { return aptr[i + lda * j]; };
...
#endif
static double tick2second(uint64_t tick) { return 1.e-9 * (double)tick; }
#endif
