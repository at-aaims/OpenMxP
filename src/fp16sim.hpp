...
inline void hgemm(int m, int n, int k, float alpha, fp16 const *a, int lda,
                  fp16 const *b, int ldb, float beta, fp16 *c, int ldc) {
    // HGEMM
...
inline void shgemm(int m, int n, int k, float alpha, fp16 const *a, int lda,
                   fp16 const *b, int ldb, float beta, float *c, int ldc) {
...
inline void hgemm(int m, int n, int k, float alpha, fp16 const *a, int lda,
    ...
    short one = 15360;   // == 1.
    short mone = -17408; // == -1.
    fjblas_gemm_r16_("N", "N", &m, &n, &k, &mone, a, &lda, b, &ldb, &one, c,
                     &ldc);
}
inline void shgemm(int, int, int, float, fp16 const *, int, fp16 const *, int,
                   float, float *, int) {
    abort();
}
#endif
...
struct fp16 {
    ...
    fp16(float t) { x = _cvtss_sh(t, 0); }
    float convert_to_float() const { return _cvtsh_ss(x); }
    explicit operator float() const { return convert_to_float(); }
...
// double rounding causes larger error in very rare case. we ignore it for
// performance
...
void hgemm_opt(int m, int n, int k, float alpha, fp16 const *a, int lda,
               fp16 const *b, int ldb, float /*beta*/, fp16 *c, int ldc);
inline void hgemm(int m, int n, int k, float alpha, fp16 const *a, int lda,
                  fp16 const *b, int ldb, float beta, fp16 *c, int ldc) {
#ifdef FP16_AVX2_EMULATION
    ...
    for (int j = 0; j < m; ++j) {
        for (int i = 0; i < n; ++i) {
            ...
            c[ldc * i + j] = (c[ldc * i + j].convert_to_float() * beta +
                              temp.convert_to_float() * alpha);
        }
    }
}

void shgemm_opt(int m, int n, int k, float alpha, fp16 const *a, int lda,
                fp16 const *b, int ldb, float /*beta*/, float *c, int ldc);
inline void shgemm(int m, int n, int k, float alpha, fp16 const *a, int lda,
                   fp16 const *b, int ldb, float beta, float *c, int ldc) {
#ifdef FP16_AVX2_EMULATION
    ...
    for (int j = 0; j < m; ++j) {
        for (int i = 0; i < n; ++i) {
            float temp = 0.f;
            for (int l = 0; l < k; ++l)
                temp = a[l * lda + j].convert_to_float() *
                           b[i * ldb + l].convert_to_float() +
                       temp;
            c[ldc * i + j] = c[ldc * i + j] * beta + temp * alpha;
        }
    }
