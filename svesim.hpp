...
static svint64_t svld1_s64(svbool_t t, int64_t const *x) {
    svint64_t r;
    for (int i = 0; i < SVE_VLEN / 8; ++i)
        r.x[i] = (t.x[8 * i] ? x[i] : 0ll);
    return r;
}
static svint64_t svmad_s64_x(svbool_t t, svint64_t a, svint64_t b,
                             svint64_t c) {
    svint64_t r;
    for (int i = 0; i < SVE_VLEN / 8; ++i)
...
static svfloat64_t svld1_vnum_f64(svbool_t t, double const *x, int vnum) {
    svfloat64_t r;
    for (int i = 0; i < SVE_VLEN / 8; ++i)
        r.x[i] = (t.x[8 * i] ? x[vnum * SVE_VLEN / 8 + i] : 0.);
    return r;
}
...
static svfloat64_t svmla_f64_x(svbool_t t, svfloat64_t a, svfloat64_t b,
                               svfloat64_t c) {
    svfloat64_t r;
    for (int i = 0; i < SVE_VLEN / 8; ++i)
        r.x[i] = (t.x[8 * i] ? a.x[i] + b.x[i] * c.x[i] : a.x[i]);
...
static svfloat64_t svmla_n_f64_x(svbool_t t, svfloat64_t a, svfloat64_t b,
                                 float c) {
    svfloat64_t r;
    for (int i = 0; i < SVE_VLEN / 8; ++i)
...
static svfloat32_t svld1_vnum_f32(svbool_t t, float const *x, int vnum) {
    svfloat32_t r;
    for (int i = 0; i < SVE_VLEN / 4; ++i)
        r.x[i] = (t.x[4 * i] ? x[vnum * SVE_VLEN / 4 + i] : 0.f);
    return r;
}
...
static svfloat32_t svmla_f32_x(svbool_t t, svfloat32_t a, svfloat32_t b,
                               svfloat32_t c) {
    svfloat32_t r;
    for (int i = 0; i < SVE_VLEN / 4; ++i)
...
static svfloat32_t svmls_f32_x(svbool_t t, svfloat32_t a, svfloat32_t b,
                               svfloat32_t c) {
    svfloat32_t r;
    for (int i = 0; i < SVE_VLEN / 4; ++i)
        r.x[i] = (t.x[4 * i] ? a.x[i] - b.x[i] * c.x[i] : a.x[i]);
...
static svfloat32_t svnmls_f32_x(svbool_t t, svfloat32_t a, svfloat32_t b,
                                svfloat32_t c) {
    svfloat32_t r;
    for (int i = 0; i < SVE_VLEN / 4; ++i)
...
static svfloat16_t svld1_vnum_f16(svbool_t t, fp16 const *x, int vnum) {
    svfloat16_t r;
    for (int i = 0; i < SVE_VLEN / 2; ++i)
        r.x[i] = (t.x[2 * i] ? x[vnum * SVE_VLEN / 2 + i] : fp16(0.f));
    return r;
}
...
static svfloat16_t svmla_f16_x(svbool_t t, svfloat16_t a, svfloat16_t b,
                               svfloat16_t c) {
    svfloat16_t r;
    for (int i = 0; i < SVE_VLEN / 2; ++i)
...
static svfloat16_t svmls_f16_x(svbool_t t, svfloat16_t a, svfloat16_t b,
                               svfloat16_t c) {
    svfloat16_t r;
    for (int i = 0; i < SVE_VLEN / 2; ++i)
        r.x[i] = (t.x[2 * i] ? a.x[i] - b.x[i] * c.x[i] : a.x[i]);
...
static svfloat16_t svnmls_f16_x(svbool_t t, svfloat16_t a, svfloat16_t b,
                                svfloat16_t c) {
    svfloat16_t r;
    for (int i = 0; i < SVE_VLEN / 2; ++i)
...
static svint16_t svld1_vnum_s16(svbool_t t, short const *x, int vnum) {
    svint16_t r;
    for (int i = 0; i < SVE_VLEN / 2; ++i)
        r.x[i] = (t.x[2 * i] ? x[vnum * SVE_VLEN / 2 + i] : (short)0);
    return r;
}
