...
template <int> struct Timer_template : Timer_template_base {
    ...
    static void dump_mp(const int /*size*/, const int /*rank*/,
                        ...
                        const char * /*filename*/ = "") {} // do nothing
};

template <> struct Timer_template<1> : Timer_template_base {
...

    {
        ...
        for (int i = 0; i < NUM_ITEMS; i++) {
            fprintf(fp, fmt, name(i), rtime(i), 100.0 * time(i) / time(TOTAL),
                    accum(i), 1e-9 * accum(i) / rtime(i));
        }
        const double flops =
            (double)(accum(GEMM_UPDATE) + accum(GEMM_PROGRESS)) /
    ...
    static void dump_mp(const int size, const int rank, const int row,
                        const int col,
	...
	static double rtime(int const i) { return tick2second(time(i)); }
...
template <> struct Timer_template<2> : Timer_template_base {
    static char const *name(int const i) {
        static const char *strs[NUM_ITEMS] = {
            "DIAG_BCAST",
            "LCOL_BCAST",
    ...
    static void dump_mp(const int /*size*/, const int rank, const int row,
                        const int col, const char *filename) {
  ...
  private:
    ...
    static void dump_vector(FILE *fp, const tvec &v, const char *s0,
                            const char *s1) {
        const int n = v.size();
        if (fp != stderr) {
    ...
    static void dump_accum(FILE *fp, const tvec &v, const char *s0,
                           const char *s1) {
        const int n = v.size();
        if (fp != stderr) {
            fprintf(fp, "bio, %d, %s%s\n", n, s0, s1);
